...
export const PATCH: RequestHandler = async ({ request, locals, params }) => {
	...
	await attributeRepo.update(locals.userId, getRequiredParam(params, "name"), parsing.data.name ?? null);

	return createJsonResponse({});
};

export const DELETE: RequestHandler = async ({ locals, params }) => {
	await attributeRepo.delete(locals.userId, getRequiredParam(params, "name"));
